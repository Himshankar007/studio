              ...
              return (
                <Card key={product.id} className="flex flex-col overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 group">
                  ...
                  <CardContent className="p-6 flex flex-col flex-grow">
                    <CardTitle className="font-headline text-xl">{t(`products.${product.id}.name`)}</CardTitle>
                    <CardDescription className="mt-1 text-sm text-muted-foreground">
                      {t('from')} {product.monastery}
                    </CardDescription>
                    <div className="flex-grow" />
                    <div className="flex justify-between items-center mt-4">
                      <p className="text-2xl font-bold text-primary">{product.price}</p>
                      <Button size="icon" aria-label={`${t('addToCart')} ${t(`products.${product.id}.name`)}`}>
